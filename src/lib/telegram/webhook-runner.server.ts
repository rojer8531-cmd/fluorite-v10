...
export async function runTelegramWebhook(
  ...
  const workPromise = (async () => {
    await (previous ?? Promise.resolve()).catch(() => {});
    ...
    try {
      ...
      const workElapsed = Date.now() - workStartedAt;
      if (workElapsed > SLOW_LOG_MS) {
        console.warn(`[${label} webhook] slow handler ${workElapsed}ms`);
      }
    }
  })()
    .finally(() => {
  ...
  if (elapsed > SLOW_LOG_MS) {
    console.warn(`[${label} webhook] slow total ${elapsed}ms`);
  }
}
